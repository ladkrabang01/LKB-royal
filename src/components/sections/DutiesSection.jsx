...
const dutyCards = [
  { icon: (
    ...
    ), title: 'มูลนิธิส่งเสริมศิลปาชีพ', body: 'ทรงริเริ่มก่อตั้ง "มูลนิธิส่งเสริมศิลปาชีพ" เพื่อสร้างอาชีพและรายได้แก่ราษฎรผู้ยากไร้ทั่วทุกภูมิภาค' },
  { icon: (
      <svg width="44" height="44" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    ), title: 'อนุรักษ์ผ้าไทย', body: 'ทรงส่งเสริมการอนุรักษ์ผ้าไทยและศิลปหัตถกรรมพื้นบ้าน ฟื้นฟูภูมิปัญญาท้องถิ่นให้คงคุณค่าสืบต่อไป' },
  { icon: (
      <svg width="44" height="44" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    ), title: 'พัฒนาชาติ', body: 'ทรงอุทิศพระวรกายในการพัฒนาการเกษตร การแพทย์ การศึกษา และสิ่งแวดล้อม เพื่อความผาสุกของปวงชนชาวไทย' },
];
...
export default function DutiesSection() {
  ...
  return (
    <section id="duties" ref={sectionRef} className="py-20 sm:py-28 px-4 sm:px-8" style={{ background: 'linear-gradient(180deg, #F5F0E8 0%, #EDE7D9 100%)' }} aria-label="พระราชกรณียกิจ">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12 sm:mb-16 fade-in-element">
          <p className="text-[#C9A84C] tracking-[0.25em] text-xs mb-3" style={{ fontFamily: "'Cinzel', serif" }}>ROYAL DUTIES</p>
          <h2 className="font-bold text-[#0F2540]" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontFamily: "'Sarabun', sans-serif" }}>พระราชกรณียกิจ<br /><span style={{ fontSize: '0.65em', fontWeight: 400, color: '#A8865A' }}>เพื่อปวงชนชาวไทย</span></h2>
        ...
        <div className="fade-in-element fade-in-delay-1 mb-12 sm:mb-14 max-w-3xl mx-auto">
          <div className="sovereign-frame">
            <img src="/4.jpeg" alt="พระราชกรณียกิจ" className="w-full h-auto block" style={{ filter: 'grayscale(10%) brightness(0.95)' }} />
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-6">
          {dutyCards.map((card, idx) => (
            <div key={idx} className={`duty-card fade-in-element fade-in-delay-${idx + 2} border border-[#C9A84C]/30 rounded-sm p-6 sm:p-8 text-center`} style={{ background: 'rgba(15,37,64,0.05)', backdropFilter: 'blur(4px)' }}>
              <div className="flex justify-center mb-4">{card.icon}</div>
              <h3 className="font-semibold text-[#0F2540] mb-3" style={{ fontSize: '1.1rem', fontFamily: "'Sarabun', sans-serif" }}>{card.title}</h3>
              <div className="w-8 h-px bg-[#C9A84C] mx-auto mb-3 opacity-60" />
              <p className="text-[#3D3D3D] leading-[1.9]" style={{ fontSize: '0.95rem' }}>{card.body}</p>
            </div>
          ))}
        </div>
      </div>
